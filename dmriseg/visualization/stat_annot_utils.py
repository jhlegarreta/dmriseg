...
def raise_expected_got(expected, for_, got, error_type=ValueError):
    ...
    if for_ is not None:
        ...
        raise error_type("Expected {}; got {} instead.".format(expected, got))
...
def add_stat_annotation(
    ...
    assert_is_in(
        text_format, ["full", "simple", "star"], label="argument `text_format`"
    )
    assert_is_in(
        comparisons_correction,
        ["bonferroni", None],
    ...
    box_structs_dic = {
        box_struct["box"]: box_struct for box_struct in box_structs
    }
    ...
    box_struct_pairs = sorted(
        box_struct_pairs, key=lambda x: abs(x[1]["x"] - x[0]["x"])
    )
    ...
    y_stack_arr = np.array(
        [
            [box_struct["x"] for box_struct in box_structs],
    ...
    for box_struct1, box_struct2 in box_struct_pairs:
        ...
        i_ymax_in_range_x1_x2 = xi1 + np.nanargmax(
            y_stack_arr[
                1,
                np.where(
                    (x1 <= y_stack_arr[0, :]) & (y_stack_arr[0, :] <= x2)
                ),
            ]
        )
        ...
        if perform_stat_test:
            ...
            test_short_name = (
                test_short_name if test_short_name is not None else ""
            )
            result = StatResult(
                "Custom statistical test",
                test_short_name,
        ...
        if verbose >= 1:
            print(
                "{} v.s. {}: {}".format(
                    label1, label2, result.formatted_output
                )
            )
        ...
        if text is not None:
            ...
            if use_fixed_offset or got_mpl_error:
                ...
                y_top_annot = ax.transData.inverted().transform(y_top_display)[
                    1
                ]
        else:
            y_top_annot = y + h
        ...
        y_stack_arr[
            1, (x1 <= y_stack_arr[0, :]) & (y_stack_arr[0, :] <= x2)
        ] = y_top_annot
