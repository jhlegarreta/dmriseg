...
def main():
    ...
    meta_channel_pkl = os.path.join(dout, "meta_data_channel_metrics.pkl")
    model_pth = os.path.join(dout, "model_latest.pth")
    meta_loss_weights_pkl = os.path.join(dout, "loss_weights.pkl")
    test_2d = False
    test_overfit = False
    batch_size = 1
    ...
    # model_name = "UNet"
    out_channels = 7  # 3  # fastigial  datasets[dataset]["n_labels"] + 1
    ...
    loss_hausdorff = HausdorffDTLoss(
        to_onehot_y=True,
        softmax=True,
        include_background=False,
        alpha=2.0,
    ...
    if os.path.isfile(meta_pth) and model_pth is not None:
        ...
        if "best_epoch" not in meta_data:
            meta_data["best_epoch"] = 0
    else:
        meta_data = dict(
            epoch=0,
            best_metric=-1,
    ...
    # ToDo
    # This can be a little bit too much if we have a lot of classes and a lot
    # of validation data samples, as it saves classes x samples epochs.
    if os.path.isfile(meta_channel_pkl) and meta_channel_pkl is not None:
        with open(meta_channel_pkl, "rb") as f:
            meta_data_channel = pickle.load(f)
        if "metric_values" not in meta_data_channel:
            meta_data_channel["metric_values"] = []
    else:
        meta_data_channel = dict(
            metric_values=[],
        )

    if (
        os.path.isfile(meta_loss_weights_pkl)
        and meta_loss_weights_pkl is not None
    ):
        with open(meta_loss_weights_pkl, "rb") as f:
            meta_data_loss_weights = pickle.load(f)
        if "loss_weights" not in meta_data_loss_weights:
            meta_data_loss_weights["loss_weights"] = []
        if "to_steal" not in meta_data_loss_weights:
            meta_data_loss_weights["to_steal"] = 0
    else:
        meta_data_loss_weights = dict(
            to_steal=0,
            loss_weights=[],
        )

    loss_weights = [0.62, 0.38]  # [1, 0.01]
    loss_fns = [loss_dice, loss_hausdorff]
    to_steal = 0.0015
    # scheduler = OptimizationScheduler(to_steal)

    start_epoch = meta_data["epoch"]
    if start_epoch != 0:
        # ToDo
        # Save the weights in the meta_data dictionary if it is finally found
        # out that the Hausdorff loss is helpful and avoid this.
        # Compute the expected weights from the start epoch.
        # delta_weights = start_epoch * to_steal
        loss_weights = [0.62, 0.38]
        # loss_weights = [loss_weights[0] - delta_weights, loss_weights[1] + delta_weights]
    ...
    metric_values_channel = meta_data_channel["metric_values"]
    meta_loss_weights = meta_data_loss_weights["loss_weights"]
    ...
    for epoch in range(start_epoch, max_epochs):
        ...
        meta_data_channel["metric_values"] = metric_values_channel
        with open(meta_channel_pkl, "wb") as f:
            pickle.dump(meta_data_channel, f)

        meta_data_loss_weights["loss_weights"] = meta_loss_weights
        meta_data_loss_weights["to_steal"] = to_steal
        with open(meta_loss_weights_pkl, "wb") as f:
            pickle.dump(meta_data_loss_weights, f)
        ...
        for ix, batch_data in enumerate(train_loader):
            ...
            with torch.cuda.amp.autocast(enabled=use_amp):
                ...
                losses = [
                    ...
                    for loss_fn, w in zip(loss_fns, loss_weights)
                ]
        ...
        if epoch % validation_interval == 0 or epoch == max_epochs - 1:
            ...
            with torch.no_grad():
                ...
                # ToDo
                # Save channel metrics over epochs to see how they evolve with
                # weights
                # ToDo
                # Save the best epoch to the meta data as wel
                _metric_values_channel = metric_c.cpu().numpy()
                metric_values_channel.append(_metric_values_channel)

                title = f"EPOCH={epoch}"
                grid = True
                _fig = boxplot_channel_metric(
                    _metric_values_channel,
                    "Dice",
                    classnames,
                    title=title,
                    grid=grid,
                ...
                if metric > best_metric:
                    ...
                    # Save the channel-wise boxplot for the best epoch.
                    fig.savefig(os.path.join(dout, "best_outputs.png"))
                    _fig.savefig(
                        os.path.join(dout, "best_dice_channel_boxplot.png")
                    )
        ...
        meta_loss_weights.append(loss_weights)
        # optimizer, loss_fns, loss_weights = scheduler(
        #    epoch, optimizer, loss_fns, loss_weights
        # )
