...
echo "Starting volume similarity computation..."
for fold in "${folds[@]}"; do
  ...
  # Rename existing vs files to sitk_vs_
  old_sitk_vs_fname=${perf_dirname}/vs.tsv
  new_sitk_vs_fname=${perf_dirname}/sitk_vs.tsv
  if [ -e "${old_sitk_vs_fname}" ]; then
      mv "${old_sitk_vs_fname}" "${new_sitk_vs_fname}"
      echo "File renamed to ${new_sitk_vs_fname}"
  else
      echo "File ${old_sitk_vs_fname} does not exist"
  fi

  old_sitk_vs_fname=${perf_dirname}/vs_stats.tsv
  new_sitk_vs_fname=${perf_dirname}/sitk_vs_stats.tsv
  if [ -e "${old_sitk_vs_fname}" ]; then
      mv "${old_sitk_vs_fname}" "${new_sitk_vs_fname}"
      echo "File renamed to ${new_sitk_vs_fname}"
  else
  ...
  python ${perf_script_dirname}/compute_volume_similarity.py \
