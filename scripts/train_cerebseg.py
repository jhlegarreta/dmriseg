...
# Run on single GPU with:
# > python train_synthseg.py
#
# Run on multi GPU with:
# > torchrun --nproc_per_node=8 --nnodes=1 --node_rank=0 train_synthseg.py
#
# Notes
# * The code is set up to restart training from the most recent epoch,
#   in case training stops for some reason (delete meta_data.pkl in ./results
#   to start from epoch 0).
# * You can observe the training progress by looking at outputs.png written
#   every `validation_epoch` to ./results
# * Best and latest models are saved to ./results
